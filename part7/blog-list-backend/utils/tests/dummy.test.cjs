...
describe("favourite blog", () => {
  ...
  test("when list has only one blog, the favourite is the blog", () => {
    assert.deepStrictEqual(
      listHelper.favouriteBlog([blogsList[0]]),
      blogsList[0],
    );
  });
...
describe("Author with most blogs", () => {
  ...
  test("when list has only one blog, it should be the author with one blog", () => {
    assert.deepStrictEqual(listHelper.mostBlogs([blogsList[0]]), {
      author: blogsList[0].author,
      blogs: 1,
    });
  });
...
describe("Author with most likes", () => {
  ...
  test("when list has only one blog, it should be the author with one blog", () => {
    assert.deepStrictEqual(listHelper.mostLikes([blogsList[0]]), {
      author: blogsList[0].author,
      likes: blogsList[0].likes,
    });
  });
